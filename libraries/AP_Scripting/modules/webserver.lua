...
JS_LIBRARY = {
   ...
   ["populate_json_objects"] = [[
      function populate_json_objects(object_name, period_ms) {
          var xhr = new XMLHttpRequest();
          xhr.open('GET', '/' + object_name + '.json');
...
function custom_board_status()
   ...
   for _,board_status in pairs(user_board_status) do
      ...
      ret = ret .. string.format("<tr><td>%s</td><td>%s</td></tr>\n", board_status.field, web.evaluate(run))
   end
   return ret
end
...
function populate_sections()
   ...
   for _,section in ipairs(sections) do
      ret = ret .. string.format("<button class=\"collapsible\">%s</button>\n<div class=\"content\">\n%s\n</div>\n", section.title, section.content)
   end
   return ret
end
