...
void AP_GPS_UBLOX::log_mon_rf(void)
{
    ...
    const uint16_t noisePerMS = blk0.noisePerMS;
    const uint16_t agcCnt = blk0.agcCnt;
    const uint8_t flags = blk0.flags;
    const int8_t ofsI = blk0.ofsI;
    const uint8_t magI = blk0.magI;
    const int8_t ofsQ = blk0.ofsQ;
    const uint8_t magQ = blk0.magQ;
    const uint8_t aPower = blk0.antPower;
    const uint8_t jamInd = (flags & 0x03U); // jammingState per spec (0..3)

    // Fill UBX1 with RF noise and AGC-like info
    struct log_Ubx1 pkt1 = {
        LOG_PACKET_HEADER_INIT(LOG_GPS_UBX1_MSG),
        time_us    : AP_HAL::micros64(),
        instance   : state.instance,
