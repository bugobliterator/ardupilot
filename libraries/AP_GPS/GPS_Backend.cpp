...
{
    if (port) {
        state.transport_timestamp_ms = port->receive_time_constraint_us(nbytes) / 1000U;
    }
}
...
void AP_GPS_Backend::check_new_itow(uint32_t itow, uint32_t msg_length)
{
    if (itow != _last_itow) {
        ...
        state.transport_timestamp_ms = local_us / 1000U;
        ...
        if (gps.get_lag(state.instance, expected_lag)) {
            float lag_s = (now - state.transport_timestamp_ms) * 0.001;
